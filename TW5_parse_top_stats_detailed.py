...
if __name__ == '__main__':
	...
	players, fights, found_healing, found_barrier, squad_comp, squad_offensive, squad_Control, enemy_Control, enemy_Control_Player, downed_Healing, uptime_Table, auras_TableIn, auras_TableOut, Death_OnTag, SethBot = collect_stat_data(args, config, log, args.anonymize)    
	...
	Nav_Bar_Items= ('<$button set="!!curTab" setTo="Overview" selectedClass="" class="btn btn-sm btn-dark" style=""> Session Overview </$button>',
					...
					'<$button set="!!curTab" setTo="SethBot" selectedClass="" class="btn btn-sm btn-dark" style=""> DPS Stats </$button>'
	)
	...
	for stat in config.stats_to_compute:
		...
		write_to_json(overall_raid_stats, overall_squad_stats, fights, players, top_total_stat_players, top_average_stat_players, top_consistent_stat_players, top_percentage_stat_players, top_late_players, top_jack_of_all_trades_players, squad_offensive, squad_Control, enemy_Control, enemy_Control_Player, downed_Healing, uptime_Table, auras_TableIn, auras_TableOut, Death_OnTag, SethBot, args.json_output_filename)
	...
	myprint(output, '<$reveal type="match" state="!!curTab" text="SethBot">')    
	myprint(output, '\n<<alert-leftbar light "🤖 Experimental DPS stats 🤖" width:60%, class:"font-weight-bold">>\n\n')
	
	myprint(output, '\n---\n')
	myprint(output, '!!! `Chunk Damage(t)` [`Ch(t)`] \n')
	myprint(output, '!!! Damage done within `t` seconds of an enemy going down \n')
	...
	output_header += '| ! <span data-tooltip="% of damage done within 2 seconds of an enemy going down">Ch(2) %</span>'
	...
	for sethBot_prof_name in SethBot:
		name = SethBot[sethBot_prof_name]['name']
		prof = SethBot[sethBot_prof_name]['profession']
		fightTime = uptime_Table[sethBot_prof_name]['duration']

		if SethBot[sethBot_prof_name]['Damage_Total'] / fightTime < 500 or fightTime * 10 < max_fightTime:
			continue

		output_string = '|'+name+' |'+' {{'+prof+'}} | '+my_value(fightTime)
		output_string += '| ☠️'
		output_string += ' | '+'<span data-tooltip="'+my_value(SethBot[sethBot_prof_name]['Downs'])+' total downs">'+'{:.2f}'.format(round(SethBot[sethBot_prof_name]['Downs'] / (fightTime / 60), 2))+'</span>'
		output_string += '| '+'<span data-tooltip="'+my_value(SethBot[sethBot_prof_name]['Kills'])+' total kills">'+'{:.2f}'.format(round(SethBot[sethBot_prof_name]['Kills'] / (fightTime / 60), 2))+'</span>'
		output_string += '| '+'<span data-tooltip="'+my_value(SethBot[sethBot_prof_name]['Damage_Total'])+' damage over '+my_value(fightTime)+' seconds">'+my_value(round(SethBot[sethBot_prof_name]['Damage_Total'] / fightTime))+'</span>'
		output_string += '| 💀'
		output_string += ' | '+'<span data-tooltip="'+my_value(SethBot[sethBot_prof_name]['Damage_Total'])+' of '+my_value(SethBot[sethBot_prof_name]['Squad_Damage_Total'])+' squad damage">'+'{:.2f}'.format(round(100 * SethBot[sethBot_prof_name]['Damage_Total'] / SethBot[sethBot_prof_name]['Squad_Damage_Total'], 2))+'%'+'</span>'
		if SethBot[sethBot_prof_name]['Chunk_Damage_Total']:
			output_string += '| '+'<span data-tooltip="'+my_value(SethBot[sethBot_prof_name]['Chunk_Damage'])+' of '+my_value(SethBot[sethBot_prof_name]['Chunk_Damage_Total'])+' chunk(2) damage">'+'{:.2f}'.format(round(100 * SethBot[sethBot_prof_name]['Chunk_Damage'] / SethBot[sethBot_prof_name]['Chunk_Damage_Total'], 2))+'%'+'</span>'
		else:
			output_string += '| 0.00%'
		if SethBot[sethBot_prof_name]['Carrion_Damage_Total']:
			output_string += '| '+'<span data-tooltip="'+my_value(SethBot[sethBot_prof_name]['Carrion_Damage'])+' of '+my_value(SethBot[sethBot_prof_name]['Carrion_Damage_Total'])+' carrion damage">'+'{:.2f}'.format(round(100 * SethBot[sethBot_prof_name]['Carrion_Damage'] / SethBot[sethBot_prof_name]['Carrion_Damage_Total'], 2))+'%'+'</span>'
		else:
			output_string += '| 0.00%'
		output_string += '| 👻'
		output_string += ' | '+'<span data-tooltip="'+my_value(SethBot[sethBot_prof_name]['Chunk_Damage_7'])+' chunk(7) and '+my_value(SethBot[sethBot_prof_name]['Carrion_Damage'])+' carrion damage of '+my_value(SethBot[sethBot_prof_name]['Damage_Total'])+' damage">'+'{:.2f}'.format(round(100 * (SethBot[sethBot_prof_name]['Chunk_Damage_7'] + SethBot[sethBot_prof_name]['Carrion_Damage']) / SethBot[sethBot_prof_name]['Damage_Total'], 2))+'%'+'</span>'
		output_string += '| '+'<span data-tooltip="'+my_value(round(SethBot[sethBot_prof_name]['Seth_Score']))+' coordination weighted DPS over '+my_value(fightTime)+' seconds">'+my_value(round(SethBot[sethBot_prof_name]['Seth_Score'] / fightTime))+'</span>' + ' |'
	
		myprint(output, output_string)

	write_SethBoth_xls(SethBot, uptime_Table, players, args.xls_output_filename)
	myprint(output, "</$reveal>\n")
	#end Seth Bot insert
